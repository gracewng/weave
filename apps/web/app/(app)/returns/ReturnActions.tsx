  ...
  return (
    <div className="mt-2 flex flex-wrap items-center gap-2">
      ...
      <span className="flex items-center gap-1">
        ...
        <button className="btn btn-save !py-1 !text-[10px]" disabled={pending || !refund} onClick={() => start(async () => { const c = Math.round(Number(refund) * 100); if (!Number.isFinite(c)) return; if (await confirmRefund(itemId, c)) { printReceipt({ title: 'Refund confirmed', subtitle: label, lines: [{ label: 'PAID', value: usd(priceCents) }, { label: 'REFUNDED', value: usd(c), saved: true }, { label: 'MONEY RECOVERED', value: usd(c), saved: true }], footer: 'CONFIRMED BY YOU · NET OF FEES', ttlMs: 5000 }); router.refresh(); } })}>Refund confirmed</button>
      </span>
      {status === 'returning' && <button className="mono text-[10px] uppercase text-ink-3 hover:text-ink" disabled={pending} onClick={() => start(async () => { if (await cancelReturn(itemId)) router.refresh(); })}>Keeping it after all</button>}
    </div>
  );
}
