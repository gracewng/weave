                ...
                {local.owned.slice(0, 3).map((o) => (
                  <div key={o.id} className="cutout p-2">
                    ...
                    <button className="btn btn-save mt-2 w-full !py-1 !text-[10px]" disabled={busy || !!decided} onClick={() => decide({ ...base(), decision: 'use_mine', ownedItemId: o.id },
                      { title: 'Purchase voided', lines: [{ label: 'INTENDED', value: priceLabel ?? 'NO PRICE' }, { label: 'STOOD IN', value: o.name.toUpperCase().slice(0, 22) }, { label: 'PAID', value: '$0.00' }, { label: 'MONEY KEPT', value: priceLabel ?? '—', saved: !!priceLabel }], footer: priceLabel ? 'ESTIMATE' : 'COUNTED, NO DOLLARS' })}>Use mine</button>
                  </div>
                ))}
              </div>
            </Receipt>
