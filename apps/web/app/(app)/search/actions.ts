...
export interface DecisionInput {
  decision: 'hold' | 'skip' | 'use_mine' | 'bought_used' | 'buy';
  ...
  ownedItemId?: string | null; actualPaidCents?: number | null; source?: string | null;
}
...
export async function recordDecision(d: DecisionInput): Promise<{ id: string; status: string; keptCents: number; confirmed: boolean } | null> {
  ...
  const row = {
    ...
    owned_item_id: d.decision === 'use_mine' ? d.ownedItemId ?? null : null, kept_cents: kept, for_other: d.forOther,
    note: d.decision === 'use_mine' ? 'used mine' : d.decision === 'bought_used' ? 'bought used' : d.decision === 'buy' ? 'bought anyway' : null,
