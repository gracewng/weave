  ...
  return (
    <form className="space-y-3" onSubmit={(e) => { e.preventDefault(); const fd = new FormData(e.currentTarget); start(async () => {
      ...
      <label className="block">
        <div className="btn btn-primary block w-full py-6 text-center text-base">📷 Snap receipt, tag, or garment</div>
      ...
      <label className="block"><div className="mono text-[10px] uppercase text-ink-3">Or describe it</div>
      ...
      <div className="mono text-[10px] text-ink-3">PHOTO = YOUR RECEIPT.</div>
    </form>
  );
}
