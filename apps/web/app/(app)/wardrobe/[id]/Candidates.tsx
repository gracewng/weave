...
import { chooseImage, findCandidates } from './actions';
...
export function Candidates({ itemId, initial, hasImage }: { itemId: string; initial: ShoppingResult[]; hasImage: boolean }) {
  ...
  return (
    <div className="mt-2">
      <div className="mono flex items-center justify-between text-[10px] uppercase text-ink-3">
        <span>{hasImage ? 'Not the right picture?' : 'Pick the product'}</span>
