...
export async function GET(req: Request) {
  ...
  const { data: items } = await admin.from('items').select('id,user_id,name,price_cents,return_by').eq('status', 'owned').gte('return_by', t).lte('return_by', c);
  ...
  for (const d of due) console.log(`[cron:returns] ${d.user_id} ${d.name} $${((d.price_cents ?? 0) / 100).toFixed(2)} by ${d.return_by}`);
  return NextResponse.json({ due: due.map((d) => ({ id: d.id, name: d.name, price_cents: d.price_cents, return_by: d.return_by })) });
}
